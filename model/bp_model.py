...
def _generate_synthetic_data() -> tuple[np.ndarray, np.ndarray]:
    ...
    # Adjusted to target healthy ranges: 110-125 systolic / 70-85 diastolic
    # For young adults (20-30), typically 110-125 / 70-80
    systolic = (
        95.0                                   # Lower baseline (was 100)
        + 0.25 * age                           # Reduced age effect (was 0.30)
        + 3.5 * bmi_offset                     # Reduced BMI effect (was 5.0)
        + 0.12 * hr                            # Reduced HR effect (was 0.15)
        - 0.08 * rmssd                         # Increased HRV effect (was -0.05)
        + 2.0 * gender_male                    # Reduced gender effect (was 4.0)
        + rng.normal(0, 6.0, size=N_SYNTHETIC) # Reduced scatter (was 8.0)
    )
    systolic = np.clip(systolic, 90, 140)      # Tighter range (was 80-200)

    diastolic = (
        62.0                                   # Lower baseline (was 65)
        + 0.15 * age                           # Reduced age effect (was 0.20)
        + 2.0 * bmi_offset                     # Reduced BMI effect (was 3.0)
        + 0.08 * hr                            # Reduced HR effect (was 0.10)
        - 0.05 * rmssd                         # Increased HRV effect (was -0.03)
        + 1.0 * gender_male                    # Reduced gender effect (was 2.0)
        + rng.normal(0, 4.0, size=N_SYNTHETIC) # Reduced scatter (was 5.0)
    )
    diastolic = np.clip(diastolic, 60, 95)     # Tighter range (was 50-130)
