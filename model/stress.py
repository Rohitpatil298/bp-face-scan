...
def estimate_stress(
    ...
    """
    # ── Guard: if RMSSD is missing, use HR-based fallback ────────────────
    if rmssd_ms is None:
        logger.warning("RMSSD unavailable — using HR-based stress estimate.")
        
        # Fallback: estimate stress from HR alone (less accurate but better than Unknown)
        # Normal resting HR: 60-80 BPM, High: >90 BPM
        if hr_bpm < 70:
            level = "Low"
            score = 30.0
            description = (
                "Your heart rate suggests a relaxed state. "
                "(Note: Limited HRV data - ensure face is clearly visible for full scan)"
            )
        elif hr_bpm < 90:
            level = "Moderate"
            score = 50.0
            description = (
                "Your heart rate indicates a moderate autonomic state. "
                "(Note: Limited HRV data - ensure face is clearly visible for full scan)"
            )
        else:
            level = "High"
            score = 70.0
            description = (
                "Your elevated heart rate may indicate stress or recent activity. "
                "(Note: Limited HRV data - ensure face is clearly visible for full scan)"
            )
